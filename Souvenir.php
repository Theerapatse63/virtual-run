    ...
    if (!empty($_FILES["file"]["name"])) {
        ...
        if (in_array($fileType, $allowTypes)) {
            if (move_uploaded_file($_FILES['file']['tmp_name'], $targetFilePath)) {
                $insert = $conn->query("INSERT INTO souvenir(souvenir_name, souvenir_price, souvenir_image, details, event_id) VALUES ('$souvenir_name', '$price', '$fileName', '$details', '$event_id')");
                if ($insert) {
