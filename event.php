    ...
    if (!empty($_FILES["file"]["name"])) {
        ...
        if (in_array($fileType, $allowTypes)) {
            if (move_uploaded_file($_FILES['file']['tmp_name'], $targetFilePath)) {
                $insert = $conn->query("INSERT INTO events(event_name, location, objective, distance, number_of_participants, limit_number, price, organizer_phone_number, event_image) 
                VALUES ('$event_name', '$location', '$objective', '$distance', '$number_of_participants', '$limit_number', '$price', '$organizer_phone_number', '$fileName')");
